...
def apply_bias_act(x, act='linear', gain=None, lrmul=1, clamp=None, bias_var='bias', trainable=True):
    ...
    return fused_bias_act(x, b=tf.cast(b, x.dtype), act=act, gain=gain, clamp=clamp, impl="ref")
...
def conv2d(x, w, up=False, down=False, resample_kernel=None, padding=0):
    ...
    w = tf.cast(w, x.dtype)
    if up:
        x = upsample_conv_2d(x, w, data_format='NCHW', k=resample_kernel, padding=padding, impl="ref")
    elif down:
        x = conv_downsample_2d(x, w, data_format='NCHW', k=resample_kernel, padding=padding, impl="ref")
    else:
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    def upsample(y):
        with tf.variable_scope('Upsample'):
            return upsample_2d(y, k=resample_kernel, impl="ref")
