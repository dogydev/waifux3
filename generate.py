...
import waifu2x_chainer.waifu2x as waifux2
#----------------------------------------------------------------------------
...
class argparser(object):
    def __init__(self):
        self.network = "runs/00005-waifux3-auto1/network-snapshot-001024.pkl"
        self.seeds = None
        self.dlatents_npz = None
...
def main(args=None):
    ...
    for image in os.listdir(args.outdir):
        waifux2.gen(os.path.join(args.outdir, image), os.path.join(args.outdir, "upscaled_"+image))


def discord(seeds, outdir):

    network = "/data2/PycharmProjects/stylegan2-ada/training-runs/00005-waifux3-auto1/network-snapshot-001024.pkl"
    seeds = seeds
    dlatents_npz = None
    truncation_psi = 1
    class_idx = None

    generate_images(network_pkl=network, seeds=seeds, dlatents_npz=dlatents_npz, truncation_psi=truncation_psi, class_idx=class_idx, outdir=outdir)

    for image in os.listdir(outdir):
        waifux2.gen(os.path.join(outdir, image), os.path.join(outdir, "upscaled_"+image))
